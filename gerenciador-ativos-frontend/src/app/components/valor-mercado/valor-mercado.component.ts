...
export class ValorMercadoComponent {
  ...
  public valorMercado: ValorMercado = new ValorMercado(0, 0, new Date().toISOString());
  ...
  consultarValoresMercado(): void {
    this.ativoFinanceiroService.consultarValoresMercado(this.ativoFinanceiro).subscribe({
      next: response => this.valoresMercado = response,
    });
  }

  incluirValorMercado(): void {
    this.ativoFinanceiroService.incluirValorMercado(this.ativoFinanceiro, this.valorMercado).subscribe({
      next: response => { this.valorMercado = response; this.consultarValoresMercado(); },
    });
  }

  removerValorMercado(ativoFinanceiro: AtivoFinanceiro, valorMercado: ValorMercado) {
    this.ativoFinanceiroService.removerValorMercado(ativoFinanceiro.id, valorMercado).subscribe({
      next: _ => { this.consultarValoresMercado(); }
    });
  }
